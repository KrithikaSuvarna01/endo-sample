...
export const LoginForm = () => { 
 ...
 if (result.status === "success") {
        ...
        navigate("/reset-password", {
          state: { userAttributes: result.userAttributes,isFirstLogin: true }, // Pass serializable attributes
        });
      }
    } catch (error) {
    ...
    return (
      <form className=" flex flex-col gap-5" onSubmit={handleLogin} > 
                ...
                <fieldset className=" flex flex-col gap-[0.3125rem] relative">
          ...
          <input id="password" name="password" type={isPasswordVisible ? 'text' : 'password'} className="  py-[0.4375rem] px-[0.875rem] w-full max-h-[8.68%] rounded-[0.625rem] border-[0.0625rem] border-[#B8A0FF]  outline-none " />
          <button
                  type="button"
                  onClick={() => {
                    setPasswordVisible(!isPasswordVisible);
                  }}
                  className="absolute inset-y-0 right-1 flex items-center pr-3 top-4 border-none focus:border-none focus:outline-none "
                >
                  {isPasswordVisible ? (
                    <EyeOff className=" h-[1.125rem] w-[1.125rem] " color="gray" />
                  ) : (
                    <Eye className="h-[1.125rem] w-[1.125rem] " color="gray" />
                  )}
                </button>
                </fieldset>
