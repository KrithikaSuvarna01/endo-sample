...
export const ResetPasswordForm = () => { 
  ...
  const { userAttributes,isFirstLogin,email } = location.state || {};
...
async function handlePasswordReset(event: React.FormEvent<HTMLFormElement>) {
  ...
  try {
    ...
    if (isFirstLogin) {
      ...
      const mutableAttributes = { ...email };
      nonMutableAttributes.forEach((attr) => delete mutableAttributes[attr]);
      const result = await resetPassword(newPassword,mutableAttributes,email);
      console.log(result)
    }
