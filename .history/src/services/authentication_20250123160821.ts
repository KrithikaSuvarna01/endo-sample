  ...
  return new Promise((resolve, reject) => {
    cognitoUser.confirmPassword(otp, "Hello@Krithika1234", {
     onSuccess: (data) => {
        ...
        // Now that the password is confirmed, get the session and user attributes
        const userSession = cognitoUser.getSignInUserSession();

        // You can access user attributes here, for example, from the session's idToken
        const userAttributes = userSession?.getIdToken().payload;
        console.log(userAttributes)

        resolve({
          userAttributes: userAttributes // Include user attributes here
        });
      },
      onFailure: reject,
    });
