...
export const resetPassword = (email: string, newPassword: string) => {
  ...
  return new Promise((resolve, reject) => {
    // Step 1: Authenticate with the temporary password to get the session
    cognitoUser.authenticateUser(new AuthenticationDetails({
      Username: email,
      Password: 'temporaryPasswordHere'  // Provide the temporary password here
    }), {
      onSuccess: (result) => {
        // Step 2: Complete the new password challenge
      ...
      onFailure: (err) => {
        console.log(err);
        reject("Authentication failed. Please check the temporary password.");
      },
    });
  });
};